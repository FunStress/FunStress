...
class SearchMusicVC: UIViewController {
    ...
    fileprivate func configureUI() {
        ...
        textFieldInsideUISearchBar?.font = UIFont(name: "Montserrat-Medium", size: 17.0)
        ...
        self.searchMusicTracks(searchText: searchString)
        self.reloadData()
    ...
    fileprivate func searchMusicTracks(searchText: String) {
...
extension SearchMusicVC: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if let text = searchBar.text, text != "" {
            ...
            self.searchMusicTracks(searchText: text)
        } else {
            searchBar.showsCancelButton = false
            self.reloadData()
        }
